...
describe('/authentications endpoint', () => {
  ...
  describe('when POST /authentications', () => {
    ...
    it('should response 400 if name not found', async () => {
      // Arrange
      const requestPayload = {
        name: 'dicoding',
        password: 'secret',
      ...
      expect(responseJson.message).toEqual('name tidak ditemukan');
    });
    ...
    it('should response 400 if login payload not contain needed property', async () => {
      ...
      expect(response.statusCode).toEqual(400);
      expect(responseJson.status).toEqual('fail');
      expect(responseJson.message).toEqual('harus mengirimkan name dan password');
    });
    ...
    it('should response 400 if login payload wrong data type', async () => {
      ...
      expect(response.statusCode).toEqual(400);
      expect(responseJson.status).toEqual('fail');
      expect(responseJson.message).toEqual('name dan password harus string');
    });
  });
  ...
  describe('when PUT /authentications', () => {
    ...
    it('should return 400 if refresh token not registered in database', async () => {
      // Arrange
      const server = await createServer(container);
      const refreshToken = await container.getInstance(AuthenticationTokenManager.name).createRefreshToken({ name: 'dicoding' });
